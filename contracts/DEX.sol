...
contract DEX is Ownable {
    ...
    public payable returns(lpAmounts memory amounts) {
        require(_token != address(0), "Enter valid address!");
        ...
        if( keccak256(abi.encodePacked(tokenAddressToName[_token])) == keccak256(abi.encodePacked("")) ) {
            ...
            tokenAddressToLiquidity[_token] = tokenAddressToLiquidity[_token].add(deltaToken);
        ...
        // if pool already exists
            ...
            tokenAddressToLiquidity[WETH] = tokenAddressToLiquidity[WETH] + msg.value;
            tokenAddressToLiquidity[_token] = tokenAddressToLiquidity[WETH] + deltaToken;

            liquidityProviderToLiquidityProvided[msg.sender][_token] = liquidityProviderToLiquidityProvided[msg.sender][_token] + deltaToken;
            liquidityProviderToLiquidityProvided[msg.sender][WETH] = liquidityProviderToLiquidityProvided[msg.sender][WETH] + msg.value;

            amounts.tokenA = amounts.tokenA + msg.value;
            amounts.tokenB = amounts.tokenB + deltaToken;
            liquidityPoolToAvailableLiquidity[WETH][_token] = amounts;
