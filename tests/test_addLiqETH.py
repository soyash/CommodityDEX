...
# Tests todo:
# 1. Check if right part of if else called
# 2. Check if initialized at right price
# 3. Liquidity added after price of commodity changes
...
def test_addLiquidityEth_existingPool(wheatToken, dex):
    ...
    print('===============================================')
    print('Current Wheat liquidity: ' + str(dex.tokenAddressToLiquidity(wheatToken.address)))
    print('===============================================')

    initTx = dex.addLiquidityETH(wheatToken.address, 1207.593, 329.003, {'from': accounts[0], 'value': '100 ether'})

    print('Wheat token liquidity added after init: ' + str(initTx.return_value))
    print('Wheat liquidity in dex after init: ' + str(dex.tokenAddressToLiquidity(wheatToken.address)))
    print('===============================================')
    ...
    print('Wheat token liquidity added after add: ' + str(addTx.return_value))
    print('Wheat liquidity in dex after add: ' + str(dex.tokenAddressToLiquidity(wheatToken.address)))
    print('===============================================')

    #print('Return values after calling add eth liquidity: ' + str(initTx.return_value + addTx.return_value))
    
    #assert dex.tokenAddressToLiquidity(WETH) == web3.toWei(400, 'ether')
    #assert dex.tokenAddressToLiquidity(wheatToken.address) == initTx.return_value + addTx.return_value 1500607902735562309900